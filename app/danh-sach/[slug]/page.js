...
function SidebarRanking({ title, movies }) {
  return <div className="rounded-2xl border border-white/10 bg-[#1a100c]/85 p-5 shadow-xl backdrop-blur"><div className="mb-2 flex items-center justify-between border-b border-white/10 pb-3"><h2 className="font-display text-[1.35rem] tracking-wide text-[#d9a94d]">{title}</h2><span className="text-[10px] font-bold uppercase tracking-[.2em] text-[#6e5c4c]">Top 10</span></div><div>{movies.slice(0, 10).map((m, idx) => <Link href={`/watch/${m.slug}`} key={m.slug || idx} className="group flex items-center gap-3 border-b border-white/[.06] py-3 last:border-0"><div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg font-display text-lg ${idx < 3 ? 'bg-[#d9a94d] text-[#1d130a]' : 'border border-white/10 bg-white/[.03] text-[#8e7966]'}`}>{idx + 1}</div><div className="relative h-16 w-12 shrink-0 overflow-hidden rounded-lg border border-white/10 bg-[#241a14]"><Image src={m.poster} alt="" fill sizes="48px" className="object-cover transition duration-300 group-hover:scale-105" /></div><div className="min-w-0 flex-1"><div className="truncate text-sm font-bold text-[#f3ead9] transition-colors group-hover:text-[#d9a94d]">{m.title}</div><div className="mt-1 truncate text-[11px] text-[#6e5c4c]">{m.originalTitle || m.year}</div><div className="mt-1 text-[11px] font-bold text-[#b23838]">{m.episodes}</div></div></Link>)}</div></div>;
}
