    ...
    <>
      <header id="header" className={`fixed top-0 z-[1000] w-full px-[4%] py-[15px] transition-all duration-300 ${scrolled ? 'bg-[#150d0a] shadow-[0_2px_15px_rgba(0,0,0,.65)]' : 'bg-gradient-to-b from-black/95 via-black/50 to-transparent'}`}>
        <div className="flex items-center justify-between gap-3 md:gap-4">
          <div className="nav-left flex min-w-0 items-center gap-4 lg:gap-[35px]">
            <Link href="/" className="logo-wrapper flex shrink-0 items-center gap-2.5 transition hover:scale-[1.03]" aria-label="NÓN LÁ">
              <img src="/image.png" alt="Logo NÓN LÁ" className="main-logo-img h-8 w-auto object-contain md:h-10" onError={(event) => { event.currentTarget.style.display = 'none'; }} />
              <span className="logo-text bg-gradient-to-r from-[#d9a94d] to-[#b23838] bg-clip-text text-[22px] font-black uppercase tracking-[2px] text-transparent md:text-[30px]">NÓN LÁ</span>
            </Link>
            <div className="filters hidden items-center gap-[18px] lg:flex xl:gap-[25px]">
          ...
          <div className="nav-right flex items-center gap-2 md:gap-3 lg:gap-4">
            <div id="search-box" className="search-box relative flex items-center">
              ...
              {searchActive && <div className="search-suggestions index-header-menu absolute right-0 top-[calc(100%+12px)] z-[1000] flex max-h-[450px] w-[92vw] max-w-[360px] flex-col gap-1 overflow-y-auto rounded-xl p-2.5 backdrop-blur-xl md:w-[320px]">
            ...
            {user && <div id="room-join-box" className="room-join-box relative flex items-center">
              <input value={roomCode} onChange={(event) => setRoomCode(event.target.value.toUpperCase())} onKeyDown={(event) => event.key === 'Enter' && joinRoom()} placeholder="Mã phòng..." maxLength={10} className={`index-header-input h-9 rounded-full border border-white bg-black/70 py-1.5 pl-4 pr-10 text-sm uppercase text-white outline-none transition-all duration-300 ${roomActive ? 'w-[170px] opacity-100' : 'w-0 opacity-0 md:w-[150px] md:opacity-100'}`} />
              <button type="button" onClick={() => { setRoomActive((value) => !value); setSearchActive(false); if (!roomActive) setTimeout(() => document.getElementById('room-join-box')?.querySelector('input')?.focus(), 0); }} title="Tham gia phòng" className="absolute right-2 z-[111] flex h-7 w-7 items-center justify-center rounded-full text-white transition hover:scale-110 hover:text-[#b23838]">↪</button>
            </div>}

            <Link href="/tai-app" className="download-app-btn flex h-9 shrink-0 items-center justify-center gap-2 rounded-full border border-white/20 bg-white/10 px-3.5 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:border-[#3DDC84] hover:bg-white/20 sm:px-4"><span className="text-[#3DDC84]">♣</span><span className="hidden sm:inline">Tải App</span></Link>
            ...
            {ready && user && <div className="user-profile group relative flex items-center gap-2 pb-5 -mb-5"><button type="button" className="flex items-center gap-2 text-white"><img src="https://upload.wikimedia.org/wikipedia/commons/0/0b/Netflix-avatar.png" alt="User" className="h-8 w-8 rounded-md object-cover md:h-9 md:w-9" /><span className="hidden max-w-[140px] truncate text-sm font-bold md:block">{user.email}</span><span className="text-xs">⌄</span></button><div className="user-profile-menu index-header-menu invisible absolute right-0 top-full z-[1000] flex min-w-[230px] translate-y-3 flex-col gap-2 rounded-md p-3 opacity-0 transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"><div className="border-b border-white/10 pb-3 text-xs text-[#aaa] break-all">{user.email}</div>{isAdmin && <button type="button" onClick={() => setAdminOpen(true)} className="flex items-center gap-2 border-b border-white/10 px-1 py-2 text-left text-sm font-bold text-[#f5c518] hover:translate-x-1">⚙ Quản lý Quảng Cáo</button>}<button type="button" onClick={logout} className="flex items-center gap-2 px-1 py-2 text-left text-sm font-bold text-[#e5e5e5] hover:translate-x-1">↪ Đăng xuất</button></div></div>}
